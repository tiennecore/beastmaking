...
export function GripsSection() {
  return (
    <View className="mb-6">
      ...
      {GRIPS.map((grip) => (
        <View key={grip.id} className="bg-stone-800 rounded-xl p-3 mb-2">
          <Text className="text-white font-bold">{grip.name}</Text>
          <Text className="text-stone-400 text-sm">{grip.type} · {grip.description}</Text>
          <Text className="text-stone-300 text-sm mt-1">{grip.keyPoints}</Text>
        </View>
      ))}
      <View className="bg-amber-900/30 rounded-xl p-3 mt-2">
